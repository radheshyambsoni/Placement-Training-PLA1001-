...
public class May16_segSieve {
    static void simpleSieve(int limit, Vector<Integer> prime) {//11
        // This method will find all primes smaller than 'limit'
        // creating a boolean array with
        boolean isPrime[] = new boolean[limit + 1];//12
    ...
    static void segmentedSieve(int n) {//100
        // Compute all primes smaller than or equal
        // to square root of n using simple sieve
        int limit = (int) (Math.floor(Math.sqrt(n)) + 1);//11
        Vector<Integer> prime = new Vector<>();
        simpleSieve(limit, prime);//11
        ...
        while (low < n) {//t
            ...
            for (int i = 0; i < prime.size(); i++) {//5
                // Find the minimum number in [low..high] that is
                // a multiple of prime.get(i) (divisible by prime.get(i))
                // For example, if low is 31 and prime.get(i) is 3,
                // we start with 33.
                int loLim = (int) (Math.floor(low / prime.get(i)) * prime.get(i));//10
                if (loLim < low)
                    loLim += prime.get(i);//12
                 ...
                 * for range
                 */
                for (int j = loLim; j < high; j += prime.get(i))//10
                    mark[j - low] = false;
            }
